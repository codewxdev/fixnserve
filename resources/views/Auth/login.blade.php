...
<div id="twoFactorModal" class="fixed inset-0 bg-gray-600 bg-opacity-75 hidden items-center justify-center z-50">
    <div class="bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm">
        ...
        <div id="twoFactorErrorMessageContainer" class="hidden bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6" role="alert">
            <p id="twoFactorErrorMessage" class="font-medium text-sm"></p>
        </div>

        <div id="qrCodeSection" class="hidden text-center mb-6">
            <p class="text-sm mb-4">Scan the QR code with your authenticator app (e.g., Google Authenticator, Authy) to set up Two-Factor Authentication.</p>
            <div id="qrcode" class="mx-auto border p-2 rounded-lg mb-4">
            </div>
            <p class="text-xs font-mono break-all p-2 bg-gray-100 rounded">Secret: <span id="twoFactorSecret"></span></p>
        </div>
        ...
        <p id="twoFAModalBack" class="mt-4 text-center text-sm font-medium hover:underline cursor-pointer" style="color: #1169FB;">
            &larr; Back to Login
        </p>
    </div>
</div>
...
<script>
    ...
    document.getElementById("loginForm").addEventListener("submit", function(e) {
        ...
        fetch("http://localhost:8000/api/auth/login", {
            ...
            .then(res => res.json())
            .then(data => {
                localStorage.setItem('api_token', data.access_token); // Assuming 'response.token' holds the value
                if (data.status === '2fa_required') {
                    // Admin/Super Admin needs to verify 2FA before login
                    tempAccessToken = data.access_token; // Keep token for potential future use or re-login flow
                    ...
                    tempAccessToken = data.access_token; // Use this token to authenticate the enable2FA API call
