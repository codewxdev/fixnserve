    ...
    <div class="p-4 sm:p-6 lg:p-8 bg-bg-primary min-h-screen flex flex-col w-full" x-data="accessMatrix()">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 sm:mb-8 gap-4 shrink-0">
            ...
            <div class="flex flex-wrap items-center gap-3 w-full md:w-auto shrink-0">
                <button @click="modals.compare = true"
                    class="btn btn-secondary px-4 py-2 flex-1 md:flex-none flex items-center justify-center gap-2 rounded-lg">
                    <i data-lucide="git-compare" class="w-4 h-4"></i> <span class="whitespace-nowrap">Compare</span>
                </button>
                <button @click="exportMatrix()"
                    class="btn btn-primary px-4 py-2 flex-1 md:flex-none flex items-center justify-center gap-2 rounded-lg shadow-lg">
        ...
        <div class="card p-0 flex flex-col flex-1 border-border-default shadow-sm overflow-hidden relative min-h-[600px]">
            ...
            <div class="p-4 border-b border-border-default bg-bg-tertiary flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
                ...
                <div class="relative w-full sm:w-80 shrink-0">
                    ...
                    <input type="text" x-model="searchQuery" placeholder="Search permissions..."
                        class="form-input w-full pl-9 h-10 text-body-sm bg-bg-primary rounded-lg border-border-default">
                </div>
            </div>
            ...
            <div class="flex-1 overflow-auto custom-scrollbar relative bg-bg-primary w-full max-h-[70vh]">
                ...
                <div x-show="isLoading"
                    class="absolute inset-0 z-50 bg-bg-primary/90 backdrop-blur-sm flex flex-col items-center justify-center">
                ...
                <!-- TABLE START -->
                <table class="w-full text-left border-separate border-spacing-0 table-fixed min-w-max">

                    <thead>
                        <tr>
                            <!-- strictly 350px width for Permissions Head -->
                            <th class="px-5 py-4 bg-bg-secondary sticky top-0 left-0 z-40 border-b border-r border-border-default shadow-[2px_2px_5px_rgba(0,0,0,0.02)] align-middle" 
                            ...
                            <!-- strictly 160px width for Role Heads -->
                            <template x-for="role in roles" :key="role.id">
                                <th class="px-2 py-4 bg-bg-secondary sticky top-0 z-30 text-center border-b border-r border-border-default align-middle" 
                                    style="width: 160px; min-width: 160px; max-width: 160px;">
                                    <div class="flex flex-col items-center justify-center gap-1 w-full overflow-hidden mx-auto">
                                        <span class="text-body-sm font-bold text-text-primary truncate w-full" :title="role.name" x-text="role.name"></span>
                                        <span x-show="role.isSystem" class="text-[9px] text-semantic-error bg-semantic-error-bg border border-semantic-error/20 px-1.5 py-0.5 rounded uppercase font-bold flex items-center justify-center gap-1 w-max"><i data-lucide="lock" class="w-2.5 h-2.5 shrink-0"></i> Immutable</span>
                                        <span x-show="!role.isSystem" class="text-[9px] text-text-secondary bg-bg-muted border border-border-strong px-1.5 py-0.5 rounded uppercase font-bold w-max">Custom</span>
                                    </div>
                                </th>
                            </template>
                        </tr>
                        ...
                        <template x-for="mod in filteredModules" :key="mod">
                            <React.Fragment>
                                ...
                                <tr class="bg-brand-primary/5">
                                    <!-- strictly 350px width -->
                                    <td class="px-5 py-3 sticky left-0 z-20 bg-brand-primary/5 border-b border-r border-border-default shadow-[2px_0_5px_rgba(0,0,0,0.02)] align-middle" 
                                        style="width: 350px; min-width: 350px; max-width: 350px;">
                                    ...
                                    <!-- strictly 160px width -->
                                    {{-- <template x-for="role in roles" :key="role.id">
                                        <td class="p-2 text-center align-middle border-b border-r border-border-default bg-brand-primary/5" 
                                            style="width: 160px; min-width: 160px; max-width: 160px;">
                                            <div x-show="role.isSystem" class="text-[10px] text-brand-primary/50 uppercase font-bold w-full text-center">System</div>

                                            <button x-show="!role.isSystem" @click="bulkToggleModule(role, mod)"
                                                class="text-[9px] font-bold uppercase tracking-wider px-2 py-1 w-[80px] mx-auto block rounded transition-colors whitespace-nowrap overflow-hidden"
                                ...
                                <template x-for="perm in getPermissionsForModule(mod)" :key="perm.id">
                                    <tr class="hover:bg-bg-secondary transition-colors group">
                                        ...
                                        <!-- strictly 350px width -->
                                        <td class="px-5 py-4 sticky left-0 z-10 bg-bg-primary group-hover:bg-bg-secondary border-b border-r border-border-default transition-colors align-middle shadow-[2px_0_5px_rgba(0,0,0,0.02)]" 
                                            style="width: 350px; min-width: 350px; max-width: 350px;">
                                        ...
                                        <!-- strictly 160px width -->
                                        <template x-for="role in roles" :key="role.id">
                                            <td class="p-0 text-center align-middle border-b border-r border-border-default bg-inherit" 
                                                style="width: 160px; min-width: 160px; max-width: 160px;">

                                                <!-- Checkbox wrapper explicitly centered -->
                                                <label class="cursor-pointer flex items-center justify-center w-full h-full min-h-[70px] m-0 transition-colors"
                                                    ...
                                                    <div class="w-6 h-6 rounded border-2 flex items-center justify-center transition-all shadow-sm mx-auto"
                                                        :class="{
        ...
        <div x-show="modals.confirm"
            class="fixed inset-0 z-[70] flex items-center justify-center bg-brand-secondary/80 backdrop-blur-sm p-4"
            x-cloak>
            <div class="card w-full max-w-lg p-0 shadow-2xl border-semantic-warning" @click.away="cancelAction()">
                <div class="p-5 border-b border-border-default bg-bg-tertiary flex items-center gap-4 rounded-t-xl">
                    <div
                        class="w-12 h-12 rounded-full bg-semantic-warning-bg text-semantic-warning flex items-center justify-center shrink-0">
                ...
                <form @submit.prevent="executeAction">
                    <div class="p-6 space-y-6">
                        ...
                        <div class="form-group mb-0">
                            ...
                            <textarea x-model="pendingAction.justification" required rows="4" cols="10"
                                class="form-input w-full text-body-sm rounded-xl" placeholder="Mandatory reason for modifying access rights..."></textarea>
                        </div>
                    </div>
                    ...
                    <div class="p-5 border-t border-border-default flex flex-col sm:flex-row justify-end gap-3 rounded-b-xl bg-bg-tertiary">
                        ...
                        <button type="submit"
                            class="btn p-2 btn-primary bg-semantic-warning border-none hover:bg-orange-500 text-white shadow-lg w-full sm:w-auto order-1 sm:order-2 px-6"
                            :disabled="!pendingAction.justification">Confirm & Execute</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                Alpine.data('accessMatrix', () => ({
                    ...
                    requestToggle(role, perm, isChecked) {
                        ...
                        this.pendingAction = {
                            roleId: role.id,
                            roleName: role.name,
                            usersAffected: role.usersAffected,
                            justification: '',
                            type: 'toggle',
                    ...
                    executeAction() {
                        ...
                        if (type === 'toggle') {
                            ...
                            if (payload.state) {
                                modPerms.forEach(pid => {
                                    if (!this.matrix[roleId].includes(pid)) this.matrix[roleId].push(pid);
                                });
                            } else {
                    ...
                    exportMatrix() {
                        alert('Exporting Security Matrix to CSV...');
                    }
                }));
            });
        </script>
