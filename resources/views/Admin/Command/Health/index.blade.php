    ...
    <div class="p-4 sm:p-6 lg:p-8 bg-bg-primary min-h-screen" x-data="systemHealth()">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 sm:mb-8 gap-4">
            <div class="w-full md:w-auto">
                ...
                <p class="text-text-secondary text-body-sm mt-1">SRE-level observability of platform infrastructure, queues,
           ...
           dark:hover:!bg-brand-primary dark:hover:!text-white disabled:opacity-50">
                ...
                <div class="flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full"
                    :class="platformStatus === 'live' ? 'bg-semantic-success-bg border border-semantic-success/20' :
                        'bg-semantic-error-bg border border-semantic-error/20'">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6 mb-6 sm:mb-8">
            <div class="card p-4 sm:p-5 relative overflow-hidden">
                <p class="text-caption font-semibold text-text-tertiary uppercase tracking-wider truncate">Platform Uptime
                    (30d)</p>
            ...
            <div class="card p-4 sm:p-5 relative overflow-hidden">
                <p class="text-caption font-semibold text-text-tertiary uppercase tracking-wider truncate">Platform P95
                    Latency</p>
                <h3 class="text-h2 font-bold text-text-primary mt-1 sm:mt-2 truncate"
            ...
            <div class="card p-4 sm:p-5 relative overflow-hidden">
                <p class="text-caption font-semibold text-text-tertiary uppercase tracking-wider truncate">Global Error Rate
                </p>
                <h3 class="text-h2 font-bold text-text-primary mt-1 sm:mt-2 truncate"
            ...
            <div class="card p-4 sm:p-5 relative overflow-hidden">
                <p class="text-caption font-semibold text-text-tertiary uppercase tracking-wider truncate">Dependency
                    Failures</p>
                <h3 class="text-h2 font-bold text-text-primary mt-1 sm:mt-2 truncate"
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
            <div class="card p-0 overflow-hidden shadow-sm border-border-default">
                <div class="p-4 sm:p-5 border-b border-border-default bg-bg-tertiary flex justify-between items-center">
                    <h3 class="text-h4 font-bold text-text-primary flex items-center gap-2"><i data-lucide="activity"
                            class="w-4 h-4 text-brand-primary"></i> API Endpoint Health</h3>
                    <button @click="fetchSummary()" class="text-text-tertiary hover:text-text-primary"><i
                            data-lucide="refresh-cw" class="w-4 h-4" :class="isLoading ? 'animate-spin' : ''"></i></button>
                </div>
                <div class="p-5 max-h-[300px] overflow-y-auto custom-scrollbar bg-bg-primary">
                    <div class="space-y-4">
                        <template x-for="(data, endpoint) in endpoints" :key="endpoint">
                            <div
                                class="flex justify-between items-center border-b border-border-default pb-3 last:border-0 last:pb-0">
                                <div>
                                    <p class="text-body-sm font-bold text-text-primary font-mono" x-text="endpoint"></p>
                                    <p class="text-[10px] text-text-secondary mt-0.5"
                                        x-text="`RPS: ${data.rps || 0} | Errors: ${data.errors || 0}`"></p>
                                </div>
            ...
            <div class="card p-0 overflow-hidden shadow-sm border-border-default">
                <div class="p-4 sm:p-5 border-b border-border-default bg-bg-tertiary flex justify-between items-center">
                    <h3 class="text-h4 font-bold text-text-primary flex items-center gap-2"><i data-lucide="clock"
                            class="w-4 h-4 text-semantic-warning"></i> Incident Timeline</h3>
                </div>
                <div class="p-5 max-h-[300px] overflow-y-auto custom-scrollbar bg-bg-primary">
                        ...
                        <template x-for="incident in incidents" :key="incident.id">
                            <div class="relative pl-6">
                                <div class="absolute w-4 h-4 rounded-full -left-[9px] top-1 outline outline-4 outline-bg-primary"
                                    :class="incident.status === 'active' ? 'bg-semantic-error animate-pulse' :
                                        'bg-text-tertiary'">
                                </div>
        ...
        <div class="card p-0 overflow-hidden shadow-sm border-border-default">
            <div
                class="p-4 sm:p-5 border-b border-border-default flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-bg-tertiary">
                <div>
                    <h3 class="text-h4 font-bold text-text-primary flex items-center gap-2"><i data-lucide="layers"
                            class="w-4 h-4 text-purple-500"></i> Background Job Queues</h3>
            ...
            <div class="overflow-x-auto custom-scrollbar bg-bg-primary">
                <table class="w-full text-left border-collapse min-w-[900px]">
                    <thead
                        class="text-caption uppercase text-text-secondary font-semibold bg-bg-secondary border-b border-border-strong">
                        <tr>
                    ...
                    <tbody class="divide-y divide-border-default">
                        <template x-for="queue in queues" :key="queue.name">
                            <tr class="hover:bg-bg-tertiary transition-colors">
                                <td class="px-6 py-4 font-mono text-body-sm font-bold text-text-primary whitespace-nowrap"
                                    x-text="queue.name"></td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span x-show="queue.status === 'active'"
                                        class="px-2 py-0.5 rounded text-[10px] font-bold bg-semantic-success-bg text-semantic-success border border-semantic-success/20 uppercase">Active</span>
                                    <span x-show="queue.status === 'throttled'"
                                        class="px-2 py-0.5 rounded text-[10px] font-bold bg-semantic-warning-bg text-semantic-warning border border-semantic-warning/20 uppercase">Throttled</span>
                                    <span x-show="queue.status === 'paused'"
                                        class="px-2 py-0.5 rounded text-[10px] font-bold bg-semantic-error-bg text-semantic-error border border-semantic-error/20 uppercase">Paused</span>
                                </td>
                                    ...
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <button @click="replayDLQ(queue)" :disabled="queue.dlq === 0"
                                            class="btn btn-sm btn-secondary text-xs transition-all duration-200 
                                               ...
                                               hover:!bg-brand-primary hover:!text-white 
                                               dark:bg-bg-tertiary dark:text-text-secondary dark:border-border-strong 
                                               dark:hover:!bg-brand-primary dark:hover:!text-white">Replay
                                            DLQ</button>
                                    </div>
                                </td>
                            </tr>
        ...
        <div x-show="showModal" style="display: none;"
            ...
            <div @click.away="showModal = false"
                class="bg-bg-primary border border-border-strong rounded-2xl shadow-2xl p-8 max-w-sm w-full transform transition-all scale-100 relative">
            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('systemHealth', () => ({
                    ...
                    async exportReport() {
                        ...
                        try {
                            const res = await fetch("{{ url('/cp/v1/metrics/export') }}", {
                            ...
                            a.href = url;
                            a.download =
                                `system-health-report-${new Date().toISOString().split('T')[0]}.pdf`;
                            ...
                            console.error(e);
                            this.openModal('Mock Export',
                                'In a real environment, this would download a PDF/CSV of the health metrics.'
                                );
                        } finally {
                    ...
                    async fetchSummary() {
                        try {
                            ...
                            const badKeys = ['p95_latency_ms', 'error_rate', 'throughput_rps',
                                'dependency_failures'
                            ];

                            for (const [key, val] of Object.entries(rawEndpoints)) {
                                if (badKeys.includes(key) || typeof val !== 'object' || val === null)
                                    continue;
                    ...
                    toggleQueue(queue) {
                        ...
                        this.openModal('Queue Updated', 'Queue ' + queue.name + ' has been ' + queue
                            .status + '.');
                    },

                    throttleQueue(queue) {
                        queue.status = 'throttled';
                        this.openModal('Queue Throttled', 'Throttling intake for ' + queue.name +
                            '. Backlog will be prioritized.');
                    },

                    replayDLQ(queue) {
                        this.openModal('Replay Initiated', 'Replaying ' + queue.dlq + ' failed jobs in ' +
                            queue.name + ' DLQ.');
                        queue.dlq = 0;
                    },
                    ...
                    triggerIncidentMode() {
                        ...
                        this.openModal('Incident Mode Activated',
                            'CRITICAL: Initializing Incident Mode. Team has been notified and auto-refresh is paused.'
                            );
                    }
                }));
            });
