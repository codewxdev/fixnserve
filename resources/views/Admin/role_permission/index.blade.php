...
@section('content')
    <div id="app" class="min-h-screen bg-slate-50/50 space-y-8 p-6 lg:p-10 font-sans text-slate-600">
        ...
        <section id="stats-section" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div id="total-roles-card"
                ...
                <div class="flex items-center justify-between mb-4">
                    <div
                        class="p-3 rounded-xl bg-indigo-50 text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300">
            ...
            <div id="total-permissions-card"
                ...
                <div class="flex items-center justify-between mb-4">
                    <div
                        class="p-3 rounded-xl bg-emerald-50 text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors duration-300">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div
                class="group bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 hover:border-blue-100 transition-all duration-300">
                <div class="flex items-center justify-between mb-4">
                    <div
                        class="p-3 rounded-xl bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors duration-300">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div
                class="group bg-gradient-to-br from-indigo-600 to-purple-700 p-6 rounded-2xl shadow-xl shadow-indigo-200 text-white">
                <div class="flex items-center justify-between mb-4">
        ...
        <section class="space-y-6">
            {{-- Toolbar --}}
            <div
                class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
                <div class="px-4 py-2">
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <main class="flex-grow min-w-0">
                    ...
                    {{-- Matrix View --}}
                    <div id="matrix-view"
                        class="bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-200 overflow-hidden relative min-h-[400px]">
                        <div id="matrix-loading"
                            class="absolute inset-0 bg-white/80 backdrop-blur-sm z-30 flex flex-col items-center justify-center">
                        ...
                        <div class="overflow-x-auto custom-scrollbar pb-4">
                            <table class="w-full text-sm text-left border-collapse">
                                <thead id="matrix-header"
                                    class="bg-slate-50 text-slate-700 sticky top-0 z-20 shadow-sm backdrop-blur-md bg-opacity-95">
    ...
    <div id="managePermissionsSlideover" class="hidden fixed inset-0 z-50 overflow-hidden"
        ...
        <div id="slideover-backdrop" onclick="window.closeManagePermissions()"
            class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity opacity-0 duration-300"></div>
        <div id="slideover-panel"
            class="fixed inset-y-0 right-0 max-w-full flex transition-transform transform translate-x-full duration-300 pl-10">
            <div class="w-screen max-w-lg">
                <div class="h-full flex flex-col bg-white shadow-2xl rounded-l-2xl">
                    ...
                    <div class="px-6 py-5 bg-white border-b border-slate-100 flex items-center justify-between">
                        ...
                        <button onclick="window.closeManagePermissions()"
                            class="rounded-full p-2 bg-slate-50 text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors">
                    ...
                    <div class="px-6 py-4 bg-slate-50/50 border-b border-slate-100 space-y-3">
                        ...
                        <div class="flex items-center justify-between text-xs text-slate-500 font-medium">
                            ...
                            <span class="text-indigo-600 cursor-pointer hover:underline"
                                onclick="document.getElementById('slideover-search-input').value = ''; window.filterSlideoverPermissions();">Clear
                                Filter</span>
                        </div>
                    </div>

                    {{-- Content --}}
                    <div id="slideover-permission-list"
                        class="flex-1 overflow-y-auto px-6 py-4 space-y-6 custom-scrollbar bg-slate-50/30">
                        {{-- Groups injected --}}
                    </div>
                                ...
                                <form id="addRoleForm" class="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" id="role_name" name="name"
                                            ...
                                            class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all">
                                    </div>
                                </form>
                            </div>
                        </div>
                                ...
                                <form id="editRoleForm" class="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" id="edit_role_name" name="name"
                                            class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                                    </div>
                                </form>
                            </div>
                        </div>
            ...
            <div
                            ...
                            <form id="addPermissionForm" class="mt-4 space-y-4">
                                <div>
                                    ...
                                    <input type="text" id="permission_name" name="name"
                                        ...
                                        class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                                </div>
                                <div>
                ...
                <div class="bg-slate-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-slate-100">
                    ...
                    <button type="button" onclick="document.getElementById('addPermissionModal').classList.add('hidden')"
                        class="mt-3 w-full inline-flex justify-center rounded-xl border border-slate-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-slate-700 hover:bg-slate-50 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                        Cancel
                    </button>
                </div>
            </div>
...
@push('scripts')
    <script>
        ...
        function showToaster(message, type = 'success') {
            ...
            toast.className =
                `${bgColor} text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3 transform translate-y-10 opacity-0 transition-all duration-300`;
        ...
        function renderPermissionCategories() {
            ...
            Object.keys(permissionsByCategory).sort().forEach(cat => {
                ...
                btn.className =
                    `category-filter-btn w-full text-left flex items-center justify-between px-3 py-2.5 rounded-lg text-sm text-slate-600 transition-colors`;
                btn.onclick = () => filterPermissions(cat);
                btn.innerHTML =
                    `<span class="truncate font-medium">${cat}</span><span class="text-xs bg-slate-100 px-2 py-0.5 rounded-full text-slate-500 ml-2">${permissionsByCategory[cat].length}</span>`;
        ...
        function renderMatrixView() {
            ...
            let hHtml =
                `<tr><th class="permission-cell px-6 py-4 text-xs font-bold uppercase tracking-wider bg-slate-50 border-r border-slate-200 sticky left-0 z-20">Permission</th>`;
            roles.forEach(r => {
            ...
            categories.forEach(cat => {
                ...
                body.innerHTML +=
                    `<tr class="bg-indigo-50/40"><td colspan="${roles.length+1}" class="px-6 py-2 text-xs font-bold text-indigo-700 uppercase tracking-widest sticky left-0 z-10">${cat}</td></tr>`;

                permissionsByCategory[cat].sort((a, b) => a.name.localeCompare(b.name)).forEach(p => {
                    let row =
                        `<tr><td class="permission-cell px-6 py-3 text-sm font-medium text-slate-700 bg-white border-r border-slate-100 sticky left-0 z-10">${p.name}</td>`;
                    roles.forEach(r => {
        ...
        function renderCardView() {
            ...
            roles.forEach(role => {
                ...
                const manageBtn =
                    `<button onclick="window.openManagePermissions('${role.id}')" class="flex-1 px-3 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition shadow-sm hover:shadow-md">Manage</button>`;
        ...
        function renderSlideoverList() {
            ...
            document.getElementById('slideover-permission-count').innerText =
                `${activeRoleForSlideover.permissions_list.length} Permissions Assigned`;

            Object.keys(permissionsByCategory).sort().forEach(cat => {
                const groupDiv = document.createElement('div');
                groupDiv.className = 'slideover-group mb-6';
                groupDiv.innerHTML =
                    `<h4 class="text-xs font-bold uppercase text-slate-500 mb-3 tracking-wider">${cat}</h4>`;
                permissionsByCategory[cat].forEach(p => {
                    const has = activeRoleForSlideover.permissions_list.includes(p.name);
                    const item = document.createElement('div');
                    item.className =
                        'slideover-item flex items-center justify-between p-3 mb-2 bg-white rounded-xl border border-slate-100 shadow-sm hover:border-indigo-200 transition-colors';
        ...
        window.filterSlideoverPermissions = function() {
            ...
            document.querySelectorAll('.slideover-group').forEach(grp => {
                const visible = Array.from(grp.querySelectorAll('.slideover-item')).some(i => i.style
                    .display !== 'none');
        ...
        document.getElementById('addPermissionForm').addEventListener('submit', async (e) => {
            ...
            if (!name || !cat) {
                showToaster('Fields required', 'error');
                return;
            }
            btn.disabled = true;
        ...
        window.switchView = function(view) {
            ...
            const base =
                "view-toggle-btn flex items-center px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200";
