...
@section('content')
    <div x-data="tokenManagement()">
        <div class="p-4 sm:p-6 lg:p-8 bg-bg-primary min-h-screen flex flex-col">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 sm:mb-8 gap-4 shrink-0">
                <div>
                    ...
                    <p class="text-text-secondary text-body-sm mt-2">Manage session tokens, device binding, and external API
                        integrations.</p>
                </div>
                ...
                <div class="flex items-center gap-3 w-full md:w-auto">
                    ...
                    <button x-show="activeTab === 'tokens'" @click="savePolicy()"
                        class="btn btn-primary w-full md:w-auto p-2 flex justify-center items-center gap-2"
                        :disabled="isSaving">
            ...
            <div class="flex border-b border-border-default mb-6">
                <button @click="activeTab = 'tokens'"
                    :class="activeTab === 'tokens' ? 'border-brand-primary text-brand-primary' :
                        'border-transparent text-text-secondary hover:text-text-primary hover:border-border-strong'"
                    class="pb-3 px-4 text-body-sm font-bold uppercase tracking-wider border-b-2 transition-colors">
                    Access & Refresh Tokens
                </button>
                <button @click="activeTab = 'apikeys'"
                    :class="activeTab === 'apikeys' ? 'border-brand-primary text-brand-primary' :
                        'border-transparent text-text-secondary hover:text-text-primary hover:border-border-strong'"
                    class="pb-3 px-4 text-body-sm font-bold uppercase tracking-wider border-b-2 transition-colors">
                    ...
                    <div class="p-5 sm:p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div class="space-y-3 lg:col-span-2">
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" x-model="config.deviceBinding"
                                ...
                                <div>
                                    <span
                                        ...
                                        Device Binding</span>
                                    <span class="block text-caption text-text-secondary mt-0.5">Tokens are invalidated if
                                        device
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" x-model="config.roleBinding"
                                    class="w-4 h-4 rounded border-border-strong text-brand-primary focus:ring-brand-primary/20 bg-bg-primary transition-colors">
                                <div>
                                    <span
                                        class="block text-body-sm font-bold text-text-primary group-hover:text-brand-primary transition-colors">Strict
                                        Role Binding</span>
                                    <span class="block text-caption text-text-secondary mt-0.5">Tokens auto-revoke
                                        immediately
                                        if RBAC roles are modified.</span>
                                </div>
                            </label>
                        </div>
                    </div>
                                ...
                                <template x-for="token in accessTokens" :key="token.id">
                                    <tr class="hover:bg-bg-tertiary transition-colors">
                                        <td class="px-5 py-4">
                                            ...
                                            <div
                                                class="font-mono text-[10px] text-text-secondary mt-1 flex items-center gap-2">
                                                <span
                                                    class="bg-bg-primary border border-border-strong px-1.5 py-0.5 rounded"
                                                    x-text="'ID: ' + token.id"></span>
                                        ...
                                        <td class="px-5 py-4">
                                            <div class="flex flex-wrap gap-1.5">
                                                <span
                                                    class="px-2 py-0.5 rounded bg-bg-primary text-text-secondary text-[10px] border border-border-default font-mono"
                                                    x-text="'Role: ' + token.role"></span>
                                                <span x-show="token.deviceBound"
                                                    class="px-2 py-0.5 rounded bg-blue-500/10 text-blue-500 border border-blue-500/20 text-[10px] font-bold"><i
                                                        data-lucide="lock" class="w-3 h-3 inline"></i> Bound</span>
                                            </div>
                                        </td>
                                        <td class="px-5 py-4">
                                            <div class="text-body-sm text-text-primary" x-text="token.expires"></div>
                                            <div class="text-caption"
                                                :class="token.isExpiring ? 'text-semantic-error font-bold' :
                                                    'text-text-tertiary'"
                                                x-text="token.timeUntil"></div>
                                        </td>
                                        <td class="px-5 py-4 text-right">
                                            <div class="flex justify-end gap-2">
                                                <button @click="viewMetadata(token)" title="Metadata"
           ...
           px-2.5 py-2">
                                                ...
                                                <button @click="rotateToken(token.id)"
                                                    class="btn btn-sm bg-transparent text-brand-primary border-brand-primary/30 hover:bg-brand-primary hover:text-white transition-colors duration-200 text-xs"
                                                    title="Rotate">
                                                    Rotate
                                                </button>
                                                <button @click="openExtendModal(token)"
                                                    class="btn btn-sm bg-transparent text-brand-primary border-brand-primary/30 hover:bg-brand-primary hover:text-white transition-colors duration-200 text-xs"
                                                    title="Extend TTL">Extend</button>
                                                <button @click="revokeToken(token.id)"
                                                    class="btn btn-sm btn-destructive text-xs" title="Revoke"><i
                                ...
                                <template x-for="key in apiKeys" :key="key.id">
                                    <tr class="hover:bg-bg-tertiary transition-colors">
                                        <td class="px-5 py-4">
                                            ...
                                            <span
                                                class="inline-block mt-1 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border"
                                                :class="key.env === 'live' ?
                                                    'bg-semantic-success-bg text-semantic-success border-semantic-success/20' :
                                                    'bg-semantic-warning-bg text-semantic-warning border-semantic-warning/20'"
                                                x-text="key.env"></span>
                                        </td>
                                        <td class="px-5 py-4 font-mono text-xs text-text-secondary" x-text="key.prefix">
                                        </td>
                                        <td class="px-5 py-4">
                                            <div class="flex flex-wrap gap-1 mb-1.5">
                                                <template x-for="scope in key.scopes">
                                                    <span
                                                        class="px-2 py-0.5 rounded-full bg-bg-primary border border-border-default text-text-secondary text-[10px]"
                                                        x-text="scope"></span>
                                                </template>
                                            </div>
                                            <div class="text-[10px] text-text-tertiary"
                                                x-text="'Rate Limit: ' + key.rateLimit + ' req/min'"></div>
                                        </td>
                                        <td class="px-5 py-4 text-body-sm text-text-primary"
                                            x-text="key.expiry || 'Never'">
                                        </td>
                                        <td class="px-5 py-4 text-right">
                                            <div class="flex justify-end gap-2">
                                                <button @click="viewLogs(key.id)" title="Usage Logs"
                                                    class="inline-flex items-center justify-center rounded-lg border
           border-slate-300 dark:border-slate-700
           bg-white dark:bg-slate-900
           text-slate-700 dark:text-slate-200
           ...
           px-2.5 py-2">
            ...
            <div x-show="modals.metadata"
                class="fixed inset-0 z-50 flex items-center justify-center bg-brand-secondary/80 backdrop-blur-sm p-4"
                x-cloak>
                <div class="card w-full max-w-md p-0 shadow-2xl" @click.away="modals.metadata = false">
                    <div
                        ...
                        <button @click="modals.metadata = false" class="text-text-tertiary"><i data-lucide="x"
                    ...
                    <div class="p-6 space-y-4">
                        <div class="bg-bg-secondary p-3 rounded border border-border-default">
                            ...
                            <span class="font-mono text-body-sm text-text-primary break-all"
                                x-text="selectedToken?.fingerprint || 'N/A'"></span>
                        </div>
                        <div class="bg-bg-secondary p-3 rounded border border-border-default">
                            <span class="text-caption text-text-secondary block">IP Address</span>
                            <span class="font-mono text-body-sm text-text-primary"
                                x-text="selectedToken?.ip || 'N/A'"></span>
                        </div>
                    </div>
                </div>
            </div>

            <div x-show="modals.extendTtl"
                class="fixed inset-0 z-50 flex items-center justify-center bg-brand-secondary/80 backdrop-blur-sm p-4"
                x-cloak>
                <div class="card w-full max-w-md p-0 shadow-2xl" @click.away="modals.extendTtl = false">
                    <div
                        ...
                        <button @click="modals.extendTtl = false" class="text-text-tertiary"><i data-lucide="x"
                                class="w-5 h-5"></i></button>
                    </div>
                    <div class="p-6 space-y-4">
                        <div
                            class="p-3 bg-semantic-warning-bg border border-semantic-warning/30 rounded text-caption text-text-primary">
                            <i data-lucide="shield-alert" class="w-4 h-4 text-semantic-warning inline mr-1"></i> Extending
                            session TTL bypasses standard security rotation. This action requires approval.
                        </div>
                        <div class="form-group mb-0">
                            <label class="form-label">Extension Duration (Days)</label>
                            <input type="number" class="form-input w-full" x-model="extendData.days" min="1"
                                max="30">
                        </div>
                        <div class="form-group mb-0">
                    ...
                    <div class="p-4 border-t border-border-default flex justify-end gap-3 rounded-b-lg bg-bg-tertiary">
                        ...
                        <button @click="submitExtendRequest()" class="btn p-2 btn-primary"
                            :disabled="!extendData.reason">Request Approval</button>
                    </div>
                </div>
            </div>

            <div x-show="modals.generateKey"
                class="fixed inset-0 z-50 flex items-center justify-center bg-brand-secondary/80 backdrop-blur-sm p-4"
                x-cloak>
                <div class="card w-full max-w-lg p-0 shadow-2xl" @click.away="modals.generateKey = false">
                    <div
                        ...
                        <button @click="modals.generateKey = false" class="text-text-tertiary"><i data-lucide="x"
                                class="w-5 h-5"></i></button>
                    </div>
                    <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="form-group mb-0">
                                ...
                                <input type="text" class="form-input w-full" x-model="newKey.name"
                        ...
                        <div class="form-group mb-0">
                            ...
                            <div class="space-y-2 p-3 border border-border-default rounded-lg bg-bg-secondary">
                                <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox"
                                        class="rounded border-border-strong text-brand-primary focus:ring-brand-primary/20 bg-bg-primary">
                                    <span class="text-body-sm text-text-primary">jobs:read</span></label>
                                <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox"
                                        class="rounded border-border-strong text-brand-primary focus:ring-brand-primary/20 bg-bg-primary">
                                    <span class="text-body-sm text-text-primary">jobs:write</span></label>
                                <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox"
                                        class="rounded border-border-strong text-brand-primary focus:ring-brand-primary/20 bg-bg-primary">
                                    <span class="text-body-sm text-text-primary">businesses:read</span></label>
                            </div>
                        </div>
                    ...
                    <div class="p-4 border-t border-border-default flex justify-end gap-3 rounded-b-lg bg-bg-tertiary">
                        ...
                        <button @click="createApiKey()" class="btn p-2 btn-primary" :disabled="!newKey.name">Generate
                            Secret</button>
                    </div>
                </div>
            </div>

            <div x-show="modals.showSecret"
                class="fixed inset-0 z-[60] flex items-center justify-center bg-brand-secondary/90 backdrop-blur-sm p-4"
                x-cloak>
                <div class="card w-full max-w-lg p-0 shadow-2xl border-semantic-success">
                    <div class="p-6 text-center space-y-4">
                        <div
                            class="w-12 h-12 rounded-full bg-semantic-success-bg text-semantic-success flex items-center justify-center mx-auto mb-2">
                            <i data-lucide="check-circle" class="w-6 h-6"></i>
                        </div>
                        <h3 class="text-h3 font-bold text-text-primary">API Key Generated</h3>
                        <p class="text-body-sm text-text-secondary">Copy this secret key now. <strong
                                class="text-semantic-error">It will never be shown again.</strong></p>
                        ...
                        <div class="flex items-center gap-2 bg-bg-tertiary border border-border-strong p-2 rounded-lg">
                            <input type="text" readonly :value="generatedSecret"
                                class="bg-transparent border-none text-text-primary font-mono text-sm w-full focus:ring-0"
                                id="secretKeyInput">
                            <button @click="copySecret()" class="btn btn-sm btn-secondary shrink-0"><i data-lucide="copy"
                                    class="w-4 h-4"></i> Copy</button>
                        </div>
                    </div>
                    <div class="p-4 border-t border-border-default flex justify-center rounded-b-lg bg-bg-tertiary">
                        <button @click="modals.showSecret = false" class="btn btn-primary w-full">I have copied it
                            securely</button>
                    </div>
                </div>
            </div>
            ...
            <div x-show="modals.confirm"
                class="fixed inset-0 bg-brand-secondary/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4"
                x-cloak>
                <div class="card max-w-sm w-full p-0 shadow-2xl border-semantic-error">
                    <div class="p-6 text-center">
                        <div
                            class="w-16 h-16 bg-semantic-error-bg text-semantic-error rounded-full flex items-center justify-center mx-auto mb-4">
                    ...
                    <div class="p-4 border-t border-border-default bg-bg-tertiary flex gap-3 rounded-b-lg">
                        ...
                        <button @click="confirmData.onConfirm(); modals.confirm = false"
                            class="btn btn-destructive flex-1 p-2">Confirm</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('tokenManagement', () => ({
                    ...
                    // Toast Notifications & Custom Modals Logic
                    toasts: [],
                    modals: {
                        metadata: false,
                        extendTtl: false,
                        generateKey: false,
                        showSecret: false,
                        confirm: false // Custom Confirmation Modal
                    },
                    confirmData: {
                        title: '',
                        message: '',
                    ...
                    // Helper: Custom Confirmation Trigger
                    ...
                    // Mock Data (Access Tokens & API Keys)
                    accessTokens: [{
                            id: '8a7f9c21',
                            user: 'Sarah Jenkins',
                            role: 'Finance Admin',
                            device: 'iOS App v2.1',
                            fingerprint: 'fx92_88a',
                            deviceBound: true,
                            ip: '194.23.11.9',
                            expires: 'Feb 10, 2026',
                            timeUntil: 'Expires in 28 days',
                            isExpiring: false
                        },
                        {
                            id: '3b2x1z99',
                    ...
                    apiKeys: [{
                        ...
                        },
                        {
                            ...
                            env: 'test',
                            prefix: 'sk_test_4m2X...',
                            scopes: ['jobs:read', 'businesses:read'],
                            rateLimit: 60,
                            expiry: 'Mar 01, 2026'
                        }
                    ],

                    // --- Updated Methods (No more browser alerts) ---

                    async savePolicy() {
                        this.isSaving = true;
                        await new Promise(r => setTimeout(r, 800)); // Simulate API
                        this.isSaving = false;
                        this.notify("Policy settings updated successfully.", "success");
                    },
                    ...
                    submitExtendRequest() {
                        ...
                        this.notify("Extension request submitted for dual approval.", "info");
                    },

                    rotateToken(id) {
                        this.triggerConfirm(
                            'Rotate Token?',
                            'The existing session token will be invalidated and re-issued immediately.',
                            () => {
                                this.notify(`Token ${id} rotated successfully.`);
                            }
                        );
                    },
                    ...
                    revokeToken(id) {
                        this.triggerConfirm(
                            'Revoke Token?',
                            'This will terminate the user session immediately. Action cannot be undone.',
                            () => {
                                this.accessTokens = this.accessTokens.filter(t => t.id !== id);
                                this.notify(`Token ${id} has been revoked.`, "error");
                            }
                        );
                    },
                    ...
                    createApiKey() {
                        ...
                        this.notify("New API Key generated.", "success");
                        this.newKey = {
                            name: '',
                            env: 'test',
                            expiry: '30',
                    ...
                    copySecret() {
                        ...
                        if (input) {
                            ...
                            this.notify("Secret copied to clipboard!");
                        }
                    },
                    ...
                    rotateApiKey(id) {
                        this.triggerConfirm(
                            'Rotate API Key?',
                            'Any existing integrations using the old secret will break instantly.',
                            () => {
                                this.notify(`Key ${id} rotated. New credentials required for sync.`,
                                    "warning");
                            }
                        );
                    },
                    ...
                    revokeApiKey(id) {
                        this.triggerConfirm(
                            'Revoke API Key?',
                            'This key will be permanently deleted and all access will be blocked.',
                            () => {
                                this.apiKeys = this.apiKeys.filter(k => k.id !== id);
                                this.notify(`API Key ${id} revoked permanently.`, "error");
                            }
                        );
                    },

                    viewLogs(id) {
                        this.notify(`Loading audit logs for Key: ${id}...`, "info");
                    }
                }));
            });
        </script>
