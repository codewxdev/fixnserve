...
@section('title', 'Privileged Governance')
...
<div class="p-4 sm:p-6 lg:p-8 bg-bg-primary min-h-screen flex flex-col" x-data="privilegedGovernance()">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8">
        <div class="xl:col-span-2 space-y-6 sm:space-y-8">
                            ...
                            <template x-for="session in activeElevations" :key="session.id">
                                <tr class="hover:bg-bg-tertiary transition-colors group">
                                    <td class="px-5 py-4">
                                        ...
                                        <div class="text-caption text-text-secondary truncate mt-0.5" x-text="'Reason: ' + session.justification"></div>
                                    </td>
                                    <td class="px-5 py-4">
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex items-center justify-between mb-1.5">
                                            <span class="text-[10px] font-bold flex items-center gap-1" :class="session.minsLeft < 15 ? 'text-semantic-error' : 'text-semantic'">
                                        ...
                                        <div class="w-full bg-bg-muted rounded-full h-1.5 overflow-hidden">
                                            <div class="h-1.5 rounded-full transition-all duration-1000" :class="session.minsLeft < 15 ? 'bg-semantic-error' : 'bg-semantic'" :style="`width: ${(session.minsLeft / session.totalMins) * 100}%`"></div>
                                        </div>
                                    </td>
                                    <td class="px-5 py-4 text-right">
                                        <button @click="confirmTerminate(session.id)" class="btn btn-sm btn-destructive text-xs py-1 opacity-0 group-hover:opacity-100 transition-opacity">Terminate Early</button>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
            ...
            <!-- PIPELINE TABS (DUAL APPROVAL / JIT REQUESTS) -->
            <div class="card p-0 overflow-hidden">
                <div class="flex border-b border-border-default bg-bg-tertiary px-2 pt-2">
                    <button @click="pipelineTab = 'dual'" :class="pipelineTab === 'dual' ? 'border-semantic text-semantic bg-bg-secondary rounded-t-md' : 'border-transparent text-text-secondary hover:text-text-primary'" class="px-5 py-3 text-body-sm font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center gap-2">
                        Four-Eyes Approvals <span class="px-1.5 py-0.5 rounded text-[9px] bg-semantic-warning/20 text-semantic-warning" x-text="dualApprovals.length"></span>
                    </button>
                    <button @click="pipelineTab = 'jit'" :class="pipelineTab === 'jit' ? 'border-brand-primary text-brand-primary bg-bg-secondary rounded-t-md' : 'border-transparent text-text-secondary hover:text-text-primary'" class="px-5 py-3 text-body-sm font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center gap-2">
                        JIT Elevation Requests <span class="px-1.5 py-0.5 rounded text-[9px] bg-brand-primary/20 text-brand-primary" x-text="jitRequests.length"></span>
                    </button>
                </div>
                        ...
                        <template x-for="req in dualApprovals" :key="req.id">
                            <div class="bg-bg-primary border border-border-default rounded-lg p-4 sm:p-5 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                                ...
                                <div class="w-full md:w-auto shrink-0 mt-2 md:mt-0">
                                    <button @click="openReviewModal(req)" class="btn btn-sm btn-primary w-full bg-semantic border-none hover:bg-orange-500 text-white shadow-lg">Review & Sign</button>
                                </div>
                            </div>
                        </template>
                    </div>
                        ...
                        <template x-for="req in jitRequests" :key="req.id">
                            <div class="bg-bg-primary border border-border-default rounded-lg p-4 sm:p-5 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                                ...
                                <div class="flex flex-row md:flex-col gap-2 w-full md:w-auto shrink-0">
                                    <button @click="confirmApproveJit(req.id)" class="btn btn-sm btn-secondary text-semantic-success border-semantic-success/30 hover:bg-semantic-success flex-1 md:flex-none justify-center">Approve</button>
                                    <button @click="openDenyJitModal(req)" class="btn btn-sm btn-destructive flex-1 md:flex-none justify-center">Deny</button>
                                </div>
                            </div>
                        </template>
                    </div>
            ...
            <div class="card p-0 overflow-hidden shadow-sm">
                ...
                <div class="p-5 space-y-4 bg-bg-secondary">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-caption font-bold bg-semantic-bg border border-semantic/30 px-2 py-1 rounded text-semantic">Active</span>
                    </div>
                </div>
            </div>
    ...
    <div x-show="modals.reviewDual" class="fixed inset-0 z-50 flex items-center justify-center bg-brand-secondary/90 backdrop-blur-sm p-4" x-cloak>
        <div class="card w-full max-w-2xl p-0 shadow-2xl border-semantic-warning" @click.away="modals.reviewDual = false">
            ...
            <div class="p-4 border-t border-border-default flex justify-between items-center rounded-b-lg bg-bg-tertiary">
                <button @click="openDenyDualModal()" class="btn btn-destructive btn-sm">Deny & Abort</button>
                <div class="flex gap-3">
                    <button @click="modals.reviewDual = false" class="btn btn-tertiary btn-sm">Cancel</button>
                    <button @click="signDualAction()" class="btn btn-primary bg-semantic border-none hover:bg-orange-500 text-white shadow-lg btn-sm flex items-center gap-2">
    ...
    <div x-show="modals.confirm" x-transition class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" x-cloak>
        <div class="card max-w-sm w-full p-5 bg-bg-primary shadow-2xl" @click.away="modals.confirm = false">
            ...
            <div class="flex justify-end gap-2">
                <button @click="modals.confirm = false" class="btn btn-tertiary">Cancel</button>
                <button @click="modals.confirm = false; confirmData.onConfirm()" class="btn btn-destructive">Confirm</button>
            </div>
        </div>
    </div>

    <!-- MODAL: INPUT PROMPT (For Denials) -->
    <div x-show="modals.input" x-transition class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" x-cloak>
        <div class="card max-w-md w-full p-5 bg-bg-primary shadow-2xl" @click.away="modals.input = false">
            ...
            <textarea x-model="inputData.value" class="form-input w-full mb-4 text-sm" rows="3" placeholder="Enter reason..."></textarea>
            <div class="flex justify-end gap-2">
                <button @click="modals.input = false" class="btn btn-tertiary">Cancel</button>
                <button @click="modals.input = false; inputData.onSubmit(inputData.value)" class="btn btn-destructive" :disabled="!inputData.value">Submit Denial</button>
            </div>
        </div>
    </div>
        ...
        <template x-for="toast in toasts" :key="toast.id">
            <div class="p-4 rounded-lg shadow-lg border flex items-center gap-3 min-w-[280px] pointer-events-auto transition-all"
                 :class="{
                    ...
                    'bg-brand-primary/10 border-brand-primary text-brand-primary': toast.type === 'info'
                 }">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('privilegedGovernance', () => ({
            ...
            inputData: { title: '', message: '', value: '', onSubmit: () => {} },
            ...
            // Helper: Input Prompt Modal
            promptInput(title, message, onSubmit) {
            ...
            async fetchData() {
                ...
                this.activeElevations = [{
                    ...
                    user: 'John Doe (You)',
                    role: 'Finance Admin',
                    justification: 'Processing delayed settlements for vendor.',
                    minsLeft: 42,
                    totalMins: 60
                }];
                ...
                this.jitRequests = [{
                    ...
                    requestedRole: 'Security Admin',
                    duration: 60,
                    justification: 'Rotating compromised API key for Zapier integration.',
                    timeAgo: '10 mins ago'
                }];
                ...
                this.dualApprovals = [
                    {
                        ...
                        description: 'Release $45,200 payout to "CleanSweep UAE"',
                        requester: 'Tariq Al Fasi',
                        justification: 'Standard weekly volume settlement.',
                        timeAgo: '5 mins ago',
                        l1_signer: 'Sarah Jenkins',
                        l2_signer: null,
                    ...
                    },
                    {
                        ...
                        description: 'Hard Disable Network Region (Russia)',
                        requester: 'Security Bot',
                        justification: 'Anomaly detected. Mass IP spoofing attempt.',
                        timeAgo: '12 mins ago',
                        l1_signer: null,
                        l2_signer: null,
            ...
            submitJitRequest() {
                ...
                this.addLog('requested', 'JIT Requested', `You requested [${this.jitForm.role}] for ${this.jitForm.duration}m.`);
                this.notify('Elevation request submitted to senior administrators.', 'info');
                this.jitForm = { role: 'Super Admin', duration: 60, justification: '' };
            },

            confirmApproveJit(id) {
                this.confirm(
                    "Grant Access?",
                    "Grant elevated access? It will automatically revoke after the requested duration.",
                    () => this.approveJit(id)
                );
            },
            ...
            approveJit(id) {
                ...
                this.activeElevations.push({
                    ...
                    totalMins: req.duration
                });

                this.addLog('approved', 'JIT Granted', `Granted [${req.requestedRole}] to ${req.requester}.`);
                this.notify(`${req.requester} has been elevated.`);
                this.$nextTick(() => lucide.createIcons());
            },

            openDenyJitModal(req) {
                this.promptInput(
                    "Deny Request",
                    `Enter the reason for denying ${req.requester}'s request:`,
                    (reason) => this.denyJit(req.id, reason)
                );
            },
            ...
            denyJit(id, reason) {
                ...
                this.notify('Request Denied', 'error');
            },

            confirmTerminate(id) {
                this.confirm(
                    "Revoke Session?",
                    "Revoke this elevation immediately? The user will lose access to privileged roles.",
            ...
            signDualAction() {
                ...
                if (!req.l1_signer) {
                    ...
                    this.notify("Level 1 signature applied.");
                    ...
                    this.notify("Action fully approved and executed!");
            ...
            openDenyDualModal() {
                this.promptInput(
                    "Abort Action",
                    "Enter abort/denial reason for the Immutable Audit Log:",
                    (reason) => this.denyDualAction(reason)
                );
            },

            denyDualAction(reason) {
                const req = this.reviewData;
                this.dualApprovals = this.dualApprovals.filter(p => p.id !== req.id);
                this.addLog('denied', 'Action Aborted', `Action ACT-${req.id} aborted. Reason: ${reason}`);
                this.notify('Action aborted/denied', 'error');
                this.modals.reviewDual = false;
            },
