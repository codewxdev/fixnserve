...
--}}
<div id="vendor-management-page" 
     class="space-y-8 p-2 md:p-4 bg-gray-100 min-h-screen" 
     x-data="{ openVendorId: null, currentTab: 'profile' }" 
     x-cloak>
    ...
    <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4">
        <div class="mb-4 sm:mb-0">
            <h1 class="text-3xl font-bold text-gray-900 flex items-center">
                <svg class="w-7 h-7 mr-3 text-indigo-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-4-4V1m4 4h10v12c0 2.21-1.79 4-4 4H4a4 4 0 01-4-4V5c0-2.21 1.79-4 4-4h12V11zM7 9h.01M17 9h.01"></path></svg>
        ...
        <div class="flex flex-wrap gap-3"> {{-- Use flex-wrap and gap for responsive buttons --}}
            <button class="flex items-center px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-500/50 transition duration-300 ease-in-out transform hover:-translate-y-0.5">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                Add New Vendor
            </button>
            <button class="flex items-center px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition duration-150 ease-in-out shadow-sm">
    ...
    <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-2xl border border-gray-100">
        ...
        <div class="sticky top-0 z-20 bg-white pt-1 pb-4 -mx-4 sm:-mx-6 px-4 sm:px-4 border-b border-gray-100 shadow-sm rounded-t-xl">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-3 items-center">
                ...
                <div class="col-span-2 sm:col-span-2 lg:col-span-3 relative">
                    ...
                    <svg class="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                </div>

                {{-- Filters (More standardized look) --}}
                <select class="col-span-2 form-select rounded-xl text-sm border-gray-300 shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <select class="col-span-1 form-select rounded-xl text-sm border-gray-300 shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div class="col-span-2 hidden lg:block">
                    <input type="date" class="form-input w-full rounded-xl text-sm border-gray-300 shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500" value="2025-12-01">
                </div>

                {{-- Reset Button (Right-aligned) --}}
                <div class="col-span-2 sm:col-span-1 flex justify-end">
                    <button class="px-4 py-2 text-sm font-semibold text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition duration-150 ease-in-out">
                        <span class="hidden sm:inline">Reset</span>
                        <svg class="w-4 h-4 sm:ml-1 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.582a8.001 8.001 0 01-15.356-2m0 0H15"></path></svg>
                    </button>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[280px] sticky left-0 bg-gray-50 z-10 border-r border-gray-200">
                            Vendor & Status
                        </th>
                        
                        {{-- Col 2: Key Metrics (Responsive) --}}
                        <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[250px] lg:min-w-[450px]">
                            Performance & Finance
                        </th>
                        
                        {{-- Col 3: Actions (Sticky Right) --}}
                        <th class="px-4 sm:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider sticky right-0 bg-gray-50 z-10 border-l border-gray-200 min-w-[120px]">
                            Actions
                        </th>
                    </tr>
                </thead>
                    ...
                    <tr class="hover:bg-indigo-50/50 transition duration-150 ease-in-out group">
                        ...
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap sticky left-0 bg-white group-hover:bg-indigo-50/50 border-r border-gray-200 z-10">
                            <div class="flex items-center">
                                <img class="h-10 w-10 rounded-lg object-cover shadow-md ring-1 ring-gray-200 mr-3 flex-shrink-0" src="https://via.placeholder.com/150/4f46e5/ffffff?text=GS" alt="Store Logo">
                                <div>
                        ...
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium sticky right-0 bg-white border-l border-gray-200 group-hover:bg-indigo-50/50 z-10">
                            <div class="flex justify-end space-x-2">
                                ...
                                <button class="p-2 rounded-full text-gray-500 hover:bg-gray-100 transition duration-150 ease-in-out hidden sm:block" title="Edit Store">
    ...
    <div x-show="openVendorId !== null" 
         ...
         class="fixed inset-0 overflow-hidden z-50" {{-- Increased Z-index to 50 --}}
                    ...
                    <div class="h-full flex flex-col bg-white shadow-xl overflow-y-scroll">
                        <div class="p-4 sm:p-6 bg-indigo-600 sticky top-0 z-20"> {{-- Sticky header for the panel --}}
                            ...
                            <div class="flex items-start justify-between">
                                ...
                                <button type="button" class="rounded-md text-indigo-200 hover:text-white transition duration-150" @click="openVendorId = null">
                        ...
                        <nav class="flex space-x-4 px-6 pt-4 border-b border-gray-200 sticky top-[68px] bg-white z-10 overflow-x-auto"> {{-- Adjusted top for sticky header --}}
                            @php
                                $tabs = ['Profile', 'Inventory', 'Pricing & Offers', 'Orders', 'Earnings', 'Admin Tools'];
                            @endphp
                            @foreach($tabs as $tab)
                        ...
                        <div class="p-6 flex-1 overflow-y-auto">
                            ...
                            <div x-show="currentTab === 'profile'" class="space-y-6">
                                ...
                                <p class="text-gray-600">This section would contain vendor contact info, registration details, KYC/business documents, and general status settings.</p>
                                {{-- Add more detailed content here --}}
                            </div>
                            <div x-show="currentTab === 'inventory'" class="space-y-6">
                                <h3 class="text-xl font-bold text-gray-900">Inventory Overview</h3>
                                <p class="text-gray-600">A high-level view of product counts, stock alerts, last sync time, and options for bulk inventory actions.</p>
                            </div>
                            <div x-show="currentTab === 'pricing-&-offers'" class="space-y-6">
                                <h3 class="text-xl font-bold text-gray-900">Pricing & Promotions</h3>
                                <p class="text-gray-600">Management interface for vendor-set prices, active discounts, flash sale eligibility, and commission structure details.</p>
                            </div>
                            <div x-show="currentTab === 'orders'" class="space-y-6">
                                <h3 class="text-xl font-bold text-gray-900">Order History</h3>
                                <p class="text-gray-600">A dedicated view of all recent and historical orders, cancellation rates, and fulfilment statistics.</p>
                            </div>
                            <div x-show="currentTab === 'earnings'" class="space-y-6">
                                <h3 class="text-xl font-bold text-gray-900">Financial Ledger</h3>
                                <p class="text-gray-600">Detailed breakdown of gross sales, commission deductions, net earnings, and withdrawal history/status.</p>
                            </div>
                            <div x-show="currentTab === 'admin-tools'" class="space-y-6">
                                <h3 class="text-xl font-bold text-gray-900">Administrative Actions</h3>
                                <p class="text-gray-600">Buttons and forms for high-privilege actions like suspending account, resetting password, forcing inventory sync, or updating delivery zones.</p>
                            </div>
                        </div>
                    </div>
                </div>
