...
@php
    $customers = [
        ...
        [
            ...
            'payment_methods' => [['type' => 'PayPal', 'email' => 's.connor@sky.net', 'icon' => 'paypal']],
            'login_history' => [],
            'active_devices' => [],
            'total_orders' => 2,
        ],
...
@section('content')
    <div class="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors duration-200 customer-row"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="mt-1 flex items-center space-x-2">
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $customer['status'] === 'verified' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div class="text-sm font-bold text-slate-900">
                                        ${{ number_format($customer['wallet_balance'], 2) }}</div>
        ...
        {{--
            ===========================================
            CREATE NEW CUSTOMER MODAL (Unchanged)
            ===========================================
        --}}
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <form id="createCustomerForm" class="p-6 space-y-6">
                        ...
                        <div>
                            <h4 class="text-sm uppercase tracking-wide text-slate-500 font-semibold mb-3">Contact Details
                            </h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                    ...
                    <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3">
                        <button type="button"
                            class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-5 py-2.5 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm">
                            Create Customer
                        </button>
                        <button type="button" onclick="closeAddCustomerModal()"
                            class="mt-3 w-full inline-flex justify-center rounded-lg border border-slate-300 shadow-sm px-5 py-2.5 bg-white text-base font-medium text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                            Cancel
                        </button>
                    </div>
                </div>
    ...
    <script>
        ...
        const getMockHistory = (type) => {
            ...
            for (let i = 1; i <= 8; i++) {
                ...
                if (type === 'Transaction') {
                    // Specific Mock Data for Transactions
                    ...
                    items.push({
                        ...
                        date: dateStr,
                        status: isCredit ? 'Credit' : 'Debit', // Using status as type
                        image: isCredit ? 'https://via.placeholder.com/50/4f46e5/ffffff?text=IN' : 'https://via.placeholder.com/50/ef4444/ffffff?text=OUT'
                    });
                } else if (type === 'Provider') {
                    // Specific Mock Data for Providers
                    items.push({
                        id: `PRV${500+i}`,
                        name: `Provider Name ${i}`,
                        service_type: 'Home Cleaning',
                        date: dateStr,
                        status: (Math.random() * 5).toFixed(1) + ' ★', // Rating as status
                        image: `https://via.placeholder.com/50?text=P${i}`
                    });
                } else {
                    // Standard Orders/Bookings
                    items.push({
                        id: `${type.charAt(0).toUpperCase()}${1000+i}`,
                        name: `${type} Item #${i} - Service`,
        ...
        const tabs = [
            ...
            { id: 'wallet', label: 'Wallet' },            // Added
            { id: 'orders', label: 'Orders' },
            { id: 'bookings', label: 'Bookings' },
            { id: 'services', label: 'Service Requests' },
            { id: 'providers', label: 'Providers' },      // Added
            { id: 'transactions', label: 'History' },     // Added
            { id: 'payments', label: 'Payment Methods' },
        ];

        let currentCustomer = null;

        // --- SEARCH & FILTER LOGIC (Main Table) ---
        ...
        function switchTab(tabId) {
            ...
            // Initialize logic if it's a list tab
        ...
        function renderContent(tabId) {
            ...
            if (tabId === 'wallet') {
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-gradient-to-r from-indigo-600 to-purple-700 p-6 rounded-2xl shadow-lg text-white">
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <h2 class="text-4xl font-extrabold tracking-tight">$${parseFloat(c.wallet_balance).toFixed(2)}</h2>
                            </div>
                    ...
                    {{-- Actions --}}
                    <div class="grid grid-cols-2 gap-4">
                        <button class="flex items-center justify-center space-x-2 w-full py-3 bg-white border border-slate-200 rounded-xl shadow-sm hover:bg-slate-50 transition-colors text-slate-700 font-semibold text-sm">
            ...
            // --- DYNAMIC LISTS (Orders, Bookings, Services, Providers, Transactions) ---
            if (['orders', 'bookings', 'services', 'providers', 'transactions'].includes(tabId)) {
                const typeMap = {
                    'orders': 'Order',
                    'bookings': 'Booking',
                ...
                <div class="space-y-4">
                    ...
                    <div class="pt-4 text-center border-t border-slate-200">
                        <button onclick="alert('Redirect to full ${title} history page')"
                            class="inline-flex items-center px-4 py-2 bg-white border border-slate-300 rounded-md font-semibold text-xs text-slate-700 uppercase tracking-widest shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-25 transition">
                            View All ${title}s
                        </button>
                    </div>
                </div>
        ...
        function applyTabFilter(tabId) {
            ...
            const filteredData = currentTabData.filter(item => {
                const matchesStatus = (statusVal === 'all') || (item.status === statusVal || (tabId === 'providers' && statusVal === 'Completed')); // Simplified filter for providers
        ...
        function renderHistoryList(tabId, data) {
            ...
            container.innerHTML = data.map(item => {
                // Different layout for different tabs
                if (tabId === 'transactions') {
                    const isCredit = item.status === 'Credit';
                    const amountClass = isCredit ? 'text-green-600' : 'text-slate-900';
                    const sign = isCredit ? '+' : '-';
                    return `
                        <div class="bg-white p-3 rounded-lg border border-slate-100 flex items-center justify-between shadow-sm">
                            <div class="flex items-center gap-3">
                                <div class="h-10 w-10 rounded-full ${isCredit ? 'bg-green-100' : 'bg-red-100'} flex items-center justify-center">
                                     <span class="font-bold ${isCredit ? 'text-green-700' : 'text-red-700'}">${isCredit ? '↓' : '↑'}</span>
                                </div>
                                <div>
                                    <p class="text-sm font-bold text-slate-900">${item.name}</p>
                                    <p class="text-xs text-slate-500">${item.date} &bull; ${item.id}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-bold ${amountClass}">${sign}$${item.price}</p>
                                <p class="text-xs text-slate-400 capitalize">${item.status}</p>
                            </div>
                        </div>
                    `;
                } 
                    ...
                    return `
                        <div class="bg-white p-3 rounded-lg border border-slate-100 flex items-center justify-between shadow-sm">
                            <div class="flex items-center gap-3">
                                <img src="${item.image}" class="h-10 w-10 rounded-full bg-slate-200 object-cover" alt="Provider">
                                <div>
                                    <p class="text-sm font-bold text-slate-900">${item.name}</p>
                                    <p class="text-xs text-slate-500">${item.service_type}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <span class="bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-0.5 rounded">${item.status}</span>
                    ...
                    // Standard Layout for Orders/Bookings
                    let statusColor = item.status === 'In Progress' ? 'bg-blue-100 text-blue-800' : 
                                      (item.status === 'Delivered' || item.status === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800');
                    return `
