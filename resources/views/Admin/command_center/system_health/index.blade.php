...
@section('content')
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div class="flex items-center gap-6">
                <div id="system-status-indicator"
                    class="flex items-center gap-2 px-4 py-2 bg-green-500/10 border border-green-500/20 rounded-full transition-colors">
        ...
        <div class="theme-bg-card rounded-xl theme-border border overflow-hidden shadow-lg">
            <div class="p-6 border-b theme-border flex justify-between items-center">
                ...
                <button onclick="fetchDashboardData()"
                    class="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded transition">
                    Force Refresh
                </button>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead class="text-xs uppercase theme-text-muted font-semibold"
    ...
    @push('scripts')
        <script>
             ...
             */
            /**
             * Main Data Fetching Logic with Fingerprint Header
             */
            async function fetchDashboardData() {
                try {
                    // 1. LocalStorage se fingerprint uthayen
                    const fingerprint = localStorage.getItem('device_fingerprint') || 'unknown';

                    // 2. Common headers prepare karein
                    const commonHeaders = {
                        "Content-Type": "application/json",
                        "Accept": "application/json",
                        "X-Device-Fingerprint": fingerprint // Aapka required header
                    };

                    // Agar aapka token bhi localStorage mein hai, toh Bearer token bhi add kar sakte hain:
                    const token = localStorage.getItem('token');
                    if (token) {
                        commonHeaders["Authorization"] = `Bearer ${token}`;
                    }

                    // 3. Sabhi API calls mein headers pass karein
