...
@section('content')
    <div class="min-h-screen theme-bg-body theme-text-main p-6 font-sans relative overflow-hidden" x-data="emergencySystem()"
                ...
                <button @click="handleOverrideClick()"
                    ...
                    <div class="text-left">
                        ...
                        <div class="text-[10px] opacity-70 theme-text-muted"
                            x-text="override.active ? 'Root Access Granted' : 'Request Access'"></div>
                    </div>
                </button>
            </div>
        </div>
            ...
            <div class="xl:col-span-1 space-y-6">
                <div class="theme-bg-card rounded-xl border theme-border shadow-lg overflow-hidden flex flex-col h-full">
                    <div class="p-5 border-b theme-border flex justify-between items-center"
                        ...
                        <div x-show="isGlobalActive"
                            class="px-2 py-1 bg-red-600 text-white text-[10px] font-bold uppercase rounded animate-pulse">
                            ⛔ Global Lock
                        </div>
                    </div>
                    ...
                    <div class="p-6 space-y-5 flex-1 relative">
                        ...
                        <div x-show="isGlobalActive"
                            class="absolute inset-0 bg-gray-900/80 z-20 flex flex-col items-center justify-center text-center p-6 backdrop-blur-sm">
                            ...
                            <p class="text-xs text-gray-400 mt-1">You cannot add specific module maintenance while global
                                maintenance is active.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 gap-2 theme-bg-body p-1 rounded-lg">
                                <button @click="maintenanceForm.scope = 'global'; maintenanceForm.target = null"
                                    :class="maintenanceForm.scope === 'global' ? 'bg-gray-700 text-white shadow' :
                                        'text-gray-500 hover:text-gray-300'"
                                    class="py-1.5 text-xs font-medium rounded transition">Global</button>
                                <button @click="maintenanceForm.scope = 'module'"
                                    :class="maintenanceForm.scope === 'module' ? 'bg-gray-700 text-white shadow' :
                                        'text-gray-500 hover:text-gray-300'"
                                    class="py-1.5 text-xs font-medium rounded transition">Module</button>
                                <button @click="maintenanceForm.scope = 'region'"
                                    :class="maintenanceForm.scope === 'region' ? 'bg-gray-700 text-white shadow' :
                                        'text-gray-500 hover:text-gray-300'"
                        ...
                        <div x-show="maintenanceForm.scope === 'module'" style="display: none;">
                            ...
                            <select x-model="maintenanceForm.target"
                                class="w-full theme-bg-body border theme-border rounded-lg text-sm px-3 py-2 theme-text-main">
                        ...
                        <div x-show="maintenanceForm.scope === 'region'" style="display: none;">
                            ...
                            <input type="number" x-model="maintenanceForm.target"
                                class="w-full theme-bg-body border theme-border rounded-lg text-sm px-3 py-2 theme-text-main"
                                placeholder="e.g. 1">
                        </div>
                        ...
                        <div>
                            ...
                            <select x-model="maintenanceForm.reason"
                                class="w-full theme-bg-body border theme-border rounded-lg text-sm px-3 py-2 mb-3 theme-text-main">
                        ...
                        <div>
                            <label class="block text-xs font-semibold theme-text-muted uppercase mb-2">Public
                                Message</label>
                            <input type="text" x-model="maintenanceForm.userMessage"
                                class="w-full theme-bg-body border theme-border rounded-lg text-sm px-3 py-2 mb-3 theme-text-main"
                                placeholder="System upgrade in progress...">
                        </div>

                        <button @click="createMaintenance()"
                            class="w-full py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-500 rounded-lg shadow-lg transition">
                            Activate Mode
                        </button>
                    </div>
                    ...
                    <div class="border-t theme-border bg-black/20">
                        ...
                        <div class="max-h-40 overflow-y-auto custom-scrollbar">
                            <template x-for="task in activeMaintenances" :key="task.id">
                                <div
                                    class="flex items-center justify-between p-3 border-b border-white/5 hover:bg-white/5 transition">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            <span class="w-2 h-2 rounded-full"
                                                :class="task.type === 'global' ? 'bg-red-500 animate-pulse' : 'bg-yellow-500'"></span>
                                            <span class="text-xs font-bold text-white uppercase"
                                                x-text="task.type"></span>
                                            <span x-show="task.type !== 'global'" class="text-[10px] text-gray-400"
                                                x-text="'(' + (task.module || task.country_id) + ')'"></span>
                                        </div>
                                        <div class="text-[10px] text-gray-500 mt-0.5" x-text="task.reason"></div>
                                    </div>
                                    <button @click="cancelMaintenance(task.id)"
                                        class="text-[10px] px-2 py-1 bg-gray-800 hover:bg-red-900 hover:text-white text-gray-400 rounded border border-gray-700 transition">
                                        End
                                    </button>
                                </div>
                            </template>
                            <div x-show="activeMaintenances.length === 0"
                                class="p-4 text-center text-[10px] text-gray-600 italic">
            ...
            <div class="xl:col-span-2 space-y-6">
                ...
                <div class="theme-bg-card rounded-xl border theme-border shadow-lg overflow-hidden">
                    <div class="p-5 border-b theme-border flex justify-between items-center"
                        ...
                        <button @click="syncKillSwitches()" class="text-xs text-blue-400 hover:text-blue-300">Sync
                            Status</button>
                    </div>
                            ...
                            <template x-for="service in services" :key="service.id">
                                <div
                                    ...
                                    <div class="absolute left-0 top-0 bottom-0 w-1 transition-colors duration-500"
                                        :class="{ 'bg-green-500': service.status === 'operational', 'bg-yellow-500': service
                                                .status === 'soft', 'bg-red-600': service.status === 'hard' }">
                                    </div>
                                    <div class="flex justify-between items-start pl-3 mb-4">
                                        <div>
                                            <h4 class="font-bold theme-text-main" x-text="service.name"></h4>
                                            <div class="text-[10px] uppercase font-mono tracking-wide mt-1"
                                                :class="{ 'text-green-500': service
                                                    .status === 'operational', 'text-red-500': service
                                                        .status !== 'operational' }"
                                                x-text="service.status.toUpperCase()"></div>
                                        </div>
                                    </div>
                                    <div class="grid grid-cols-2 gap-3 pl-3">
                                        <button @click="openKillModal(service, 'soft')"
                                            :disabled="service.status === 'soft'"
                                            class="py-2 px-3 rounded text-xs font-bold border transition disabled:opacity-50 disabled:cursor-not-allowed"
                                            :class="service.status === 'soft' ?
                                                'bg-yellow-500/10 text-yellow-500 border-yellow-500/30' :
                                                'bg-yellow-500/20 text-yellow-500 border-yellow-500/30 hover:bg-yellow-500 hover:text-gray-900'">Soft
                                            Kill</button>
                                        <button @click="openKillModal(service, 'hard')"
                                            :disabled="service.status === 'hard'"
                                            class="py-2 px-3 rounded text-xs font-bold border transition disabled:opacity-50 disabled:cursor-not-allowed"
                                            :class="service.status === 'hard' ?
                                                'bg-red-500/10 text-red-500 border-red-500/30' :
                                                'bg-red-500/20 text-red-500 border-red-500/30 hover:bg-red-600 hover:text-white'">Hard
                                            Kill</button>
                                    </div>
                                </div>
                            </template>
                ...
                <div class="theme-bg-card rounded-xl border theme-border shadow-lg overflow-hidden flex flex-col h-64">
                    <div class="p-4 border-b theme-border flex justify-between items-center"
                        ...
                        <button @click="fetchLogs()"
                            class="text-[10px] theme-text-muted hover:text-white underline">Refresh</button>
                    </div>
                            ...
                            <tbody class="divide-y" style="border-color: rgba(var(--border-color), 0.5);">
                                <template x-for="log in logs" :key="log.id">
                                    <tr class="hover:bg-white/5 transition">
                                        <td class="p-2 text-gray-400 font-mono whitespace-nowrap"
                                            x-text="formatTime(log.created_at)"></td>
                                        ...
                                        <td class="p-2">
                                            <span class="px-1.5 py-0.5 rounded border inline-block w-full text-center"
                                                :class="getLogBadgeClass(log.action)" x-text="log.action"></span>
                                        </td>
                                    </tr>
                                </template>
        ...
        <div x-show="killModal.open"
            ...
            style="display: none;">
            <div class="theme-bg-card w-full max-w-lg rounded-2xl border border-red-500/50 shadow-2xl p-6"
                ...
                <textarea x-model="killModal.justification" placeholder="Required: Reason for kill..."
                    class="w-full theme-bg-body border theme-border rounded-lg p-3 text-sm theme-text-main h-24 mb-4"></textarea>
                <div class="flex justify-end gap-3">
                    <button @click="killModal.open = false" class="px-4 py-2 text-sm theme-text-muted">Cancel</button>
                    <button @click="executeKill()" :disabled="!killModal.justification"
                        class="px-4 py-2 bg-red-600 text-white rounded font-bold hover:bg-red-700">Execute</button>
                </div>
            </div>
        </div>
        ...
        <div x-show="overrideModal.open"
            class="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm"
            style="display: none;">
            <div class="theme-bg-card w-full max-w-md rounded-2xl border border-red-600 shadow-red-900/20 shadow-2xl p-6"
                @click.away="overrideModal.open = false">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-semibold theme-text-muted uppercase mb-2">Reason</label>
                        <select x-model="overrideModal.reason"
                            class="w-full theme-bg-body border theme-border rounded-lg text-sm px-3 py-2 theme-text-main">
                            <option value="Order service upgrade">Order service upgrade</option>
                    ...
                    <div>
                        <label class="block text-xs font-semibold theme-text-muted uppercase mb-2">Duration
                            (Minutes)</label>
                        <input type="number" x-model="overrideModal.duration"
                            class="w-full theme-bg-body border theme-border rounded-lg text-sm px-3 py-2 theme-text-main"
                            min="5" max="120">
                    </div>
                </div>
                <div class="flex justify-end gap-3 mt-6">
                    <button @click="overrideModal.open = false"
                        class="px-4 py-2 text-sm theme-text-muted hover:text-white">Cancel</button>
                    <button @click="submitOverride()"
                        class="px-4 py-2 bg-red-600 text-white rounded font-bold hover:bg-red-700 shadow-lg">Activate
                        Override</button>
                </div>
            </div>
        </div>
    ...
    window.emergencySystem = function() {
        return {
            ...
            async callApi(endpoint, method, body = null) {
                ...
                // LocalStorage se fingerprint uthayen
                ...
                const options = {
                    ...
                    headers: {
                        ...
                        // Fingerprint header yahan add kar diya
            ...
            async syncMaintenance() {
                ...
                if (res && res.data) {
                    ...
                    this.activeMaintenances = res.data.filter(m => m.status === 'active' || m.status ===
                        'scheduled');
            ...
            async createMaintenance() {
                ...
                const payload = {
                    ...
                    country_id: this.maintenanceForm.scope === 'region' ? this.maintenanceForm.target :
                        null,
            ...
            getLogBadgeClass(action) {
                if (action.includes('ACTIVATED') || action.includes('HARD') || action.includes('SOFT'))
                return 'bg-red-500/10 text-red-400 border-red-500/30';
                if (action.includes('TERMINATED') || action.includes('RESTORE') || action.includes('CANCEL'))
