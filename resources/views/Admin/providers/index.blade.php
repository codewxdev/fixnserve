...
@section('content')
            ...
            @php
                $stats = [
                    ...
                    [
                        ...
                        'value' => $providers->where('status', 'active')->count() ?? 0,
                    ...
                    [
                        'title' => 'Suspended',
                        'value' => $providers->where('status', 'suspend')->count() ?? 0,
                        'icon' => 'fas fa-gavel',
                        'color' => 'red',
                    ],
                                        ...
                                        <div class="flex items-center">
                                            <div class="relative">
                                                <img class="h-10 w-10 rounded-full object-cover shadow-sm ring-1 ring-gray-100"
                                                    src="{{ $image }}" alt="{{ $provider->name }}">

                                                {{-- FIXED: PERFECT CIRCLE STAR ICON --}}
                                                @if ($hasSub)
                                                    <div class="absolute -top-1 -right-1 h-4 w-4 bg-yellow-400 text-white rounded-full border-2 border-white shadow-sm flex items-center justify-center"
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900 search-name">
                                                    {{ $provider->name }}</div>
                                                    ...
                                                    <div class="py-1">
                                                        <a href="#"
                                                            class="flex items-center px-4 py-2 text-sm text-green-700 hover:bg-green-50"><i
                                                                class="fas fa-check-circle mr-3 w-4"></i> Approve</a>
                                                        <a href="#"
                                                            class="flex items-center px-4 py-2 text-sm text-red-700 hover:bg-red-50"><i
                                                                class="fas fa-times-circle mr-3 w-4"></i> Suspend</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <div id="details-slide-over"
        class="slide-over-panel fixed top-0 right-0 h-full w-full sm:w-4/5 lg:w-3/5 xl:w-2/5 bg-white shadow-2xl z-50 overflow-y-auto">
        <div class="flex flex-col h-full">
            <div
                class="sticky top-0 bg-white p-6 border-b border-gray-200 flex justify-between items-center shadow-sm z-10">
            ...
            <div
                ...
                <form id="createProviderForm" class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="name"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm p-2.5 border"
                ...
                <div class="p-6 border-t border-gray-200 flex justify-end bg-gray-50">
                    <button type="button" onclick="closeCreateProviderModal()"
                        class="mr-3 px-4 py-2 text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 shadow-sm transition">Cancel</button>

                    <button type="button" id="submitProviderBtn"
                        class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-md flex items-center transition">
                        <svg id="providerLoadingIcon" class="hidden animate-spin -ml-1 mr-2 h-4 w-4 text-white"
...
@push('scripts')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll("#searchInput, #categoryFilter, #kycFilter, #subscriptionFilter").forEach(
                el => {
      ...
      function generateSlideOverContent(data) {
            // 1. Existing Skills Logic
            const skillsHtml = data.skills.length > 0 ?
                data.skills.slice(0, 5).map(skill =>
                    `<span class="px-2 py-1 bg-blue-50 text-blue-700 text-xs font-semibold rounded border border-blue-100">${skill}</span>`
                ).join('') :
                '<span class="text-xs text-gray-500 italic">No skills listed (N/A)</span>';

            // 2. Existing Languages Logic
            const languagesHtml = data.languages.length > 0 ?
                data.languages.map(l => `<li>${l}</li>`).join('') :
                '<li class="italic text-gray-400">N/A</li>';

            // 3. Existing Payment Logic
            const paymentHtml = data.payment_methods.length > 0 ?
                data.payment_methods.map(pm =>
                    `<span class="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-medium bg-white text-gray-700 border border-gray-200 shadow-sm"><i class="fas fa-credit-card mr-1.5 text-gray-400"></i> ${pm}</span>`
                ).join('') :
                '<span class="text-xs text-gray-500 italic">N/A</span>';

            // --- 4. NEW: KYC Documents Logic (Added) ---
            // Backend se 'data.kyc_documents' expect kar raha hai (Array of objects: { name: 'ID Card', url: '...', type: 'PDF/IMG' })
            let kycHtml = '';
            if (data.kyc_documents && data.kyc_documents.length > 0) {
                kycHtml = `<div class="space-y-3">` + 
                    data.kyc_documents.map(doc => `
                    <div class="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all">
                        <div class="flex items-center space-x-3">
                            <div class="h-10 w-10 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center">
                                <i class="fas fa-file-alt text-lg"></i>
                            </div>
                            <div class="flex flex-col">
                                <p class="text-sm font-semibold text-gray-800 leading-tight">${doc.name || 'Document Name'}</p>
                                <span class="text-[10px] text-gray-500 uppercase tracking-wide">${doc.type || 'FILE'}</span>
                            </div>
                        </div>
                        <a href="${doc.url}" target="_blank" class="flex items-center px-3 py-1.5 bg-gray-100 hover:bg-indigo-600 hover:text-white text-gray-700 text-xs font-medium rounded transition-colors duration-200 group">
                            Review <i class="fas fa-external-link-alt ml-1.5 opacity-60 group-hover:opacity-100"></i>
                        </a>
                    </div>
                `).join('') + `</div>`;
            } else {
                kycHtml = `
                <div class="flex flex-col items-center justify-center p-5 bg-gray-50 border border-dashed border-gray-300 rounded-lg text-center">
                    <div class="bg-white p-2 rounded-full mb-2 shadow-sm"><i class="fas fa-folder-open text-gray-400 text-lg"></i></div>
                    <span class="text-xs text-gray-500 font-medium">No KYC Documents Submitted</span>
                </div>`;
            }

            // 5. Existing Subscription Logic
            ...
            return `
                <div class="space-y-8 pb-10">
                    ...
                    {{-- NEW SECTION: KYC Documents (Inserted Here) --}}
                    <div class="border-t border-gray-100 pt-6">
                        <div class="flex justify-between items-center mb-4">
                            <h4 class="text-sm font-bold text-gray-900 uppercase">KYC Verification Documents</h4>
                            <span class="text-xs bg-indigo-50 text-indigo-700 px-2 py-1 rounded-full font-medium">Confidential</span>
                        </div>
                        ${kycHtml}
                    </div>
