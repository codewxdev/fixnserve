                ...
                <div class="theme-bg-card rounded-xl shadow-sm border theme-border overflow-hidden">
                    <div class="px-6 py-4 border-b theme-border flex justify-between items-center"
                        ...
                        <h3 class="font-semibold theme-text-main flex items-center gap-2">
                            <i data-lucide="network" class="w-4 h-4" style="color: rgb(var(--brand-primary));"></i> IP
                            Access Rules
                        </h3>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm theme-text-muted">
                            ...
                            <tbody id="ip-rules-tbody" class="divide-y theme-border"
                                style="border-color: rgb(var(--border-color));">
                                <tr>
                                    <td colspan="5" class="px-6 py-4 text-center">Loading rules...</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="theme-bg-card rounded-xl shadow-sm border theme-border overflow-hidden">
                    <div class="px-6 py-4 border-b theme-border" style="background-color: rgba(var(--bg-body), 0.5);">
                        <h3 class="font-semibold theme-text-main flex items-center gap-2">
                            <i data-lucide="globe" class="w-4 h-4" style="color: rgb(var(--brand-primary));"></i>
                            Geo-Blocking
                        </h3>
                    </div>
                    ...
                    <div class="p-4 border-b theme-border">
                        <div class="relative">
                            <i data-lucide="search"
                                class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 theme-text-muted"></i>
                    ...
                    <div class="p-4 border-t theme-border text-center" style="background-color: rgba(var(--bg-body), 0.5);">
                        ...
                        <div class="inline-flex theme-bg-body rounded-lg border theme-border p-1">
                            <button onclick="updateDefaultPolicy('allow_all')" id="btn-allow-all"
                                class="px-3 py-1 text-xs font-bold rounded shadow-sm transition theme-text-muted">ALLOW
                                ALL</button>
                            <button onclick="updateDefaultPolicy('deny_all')" id="btn-deny-all"
                                class="px-3 py-1 text-xs font-bold rounded shadow-sm transition theme-text-muted">DENY
                                ALL</button>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        // API Base URL (Relative for dynamic environment compatibility)
        const BASE_URL = '/api/admin/security/network';
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // Wire up search filter for Geo Rules
            const geoSearchInput = document.getElementById('geo-search');
            if(geoSearchInput) {
                geoSearchInput.addEventListener('input', filterGeoRules);
            }
        });
        ...
        async function fetchIpRules() {
            ...
            tbody.innerHTML = '<tr><td colspan="5" class="px-6 py-4 text-center theme-text-muted">Loading rules...</td></tr>';

            const res = await apiRequest(`${BASE_URL}/ip-rules`);
            if (!res) return;

            // FIX: Controller returns data wrapped inside 'iprules' key
            const rules = res?.data?.iprules || res?.iprules || [];
            
            tbody.innerHTML = ''; 
            
            if (!Array.isArray(rules) || rules.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" class="px-6 py-4 text-center theme-text-muted">No IP rules found.</td></tr>';
                return;
            }

            rules.forEach(rule => {
                const isAllow = rule.type === 'allow';
                const typeBadge = isAllow 
                ...
                const rowClass = isAllow ? '' : `style="background-color: rgba(239, 68, 68, 0.05);"`;
                ...
                tbody.innerHTML += `
                    <tr class="hover:bg-white/5 transition-colors" ${rowClass}>
                        ...
                        <td class="px-6 py-3">
                            <span class="px-2 py-0.5 rounded theme-bg-body theme-text-muted text-xs border theme-border uppercase">${rule.applies_to || 'all'}</span>
                        </td>
                        <td class="px-6 py-3 theme-text-muted text-xs">${rule.comment || '-'}</td>
                        <td class="px-6 py-3 text-right">
                            <button onclick="deleteIpRule(${rule.id})" class="theme-text-muted hover:text-red-500 transition"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                        </td>
                    </tr>
                `;
            });
        ...
        async function submitIpRule(event) {
            ...
            const res = await apiRequest(`${BASE_URL}/ip-rules`, 'POST', payload);
            if (res) {
                closeIpModal();
                document.getElementById('ip-rule-form').reset();
                fetchIpRules(); // Table auto-refresh
            }
        }

        async function deleteIpRule(id) {
            if(!confirm("Are you sure you want to delete this rule?")) return;
            const res = await apiRequest(`${BASE_URL}/ip-rules/${id}`, 'DELETE');
            if(res) {
                fetchIpRules(); // Table auto-refresh
            }
        }
        ...
        async function fetchGeoRules() {
            ...
            listContainer.innerHTML = '<p class="text-center text-sm py-4 theme-text-muted">Loading geo rules...</p>';

            const res = await apiRequest(`${BASE_URL}/geo-rules`);
            if (!res) return;

            const rules = res?.data?.geoRules || res?.geoRules || [];
            
            listContainer.innerHTML = ''; 

            let defaultRule = rules.find(r => r.is_default || r.country_code === '*');
            updateDefaultUI(defaultRule ? defaultRule.status : 'allowed');

            const specificRules = rules.filter(r => r.country_code !== '*');

            if (specificRules.length === 0) {
                listContainer.innerHTML = '<p class="text-center text-sm py-4 theme-text-muted">No specific country rules configured.</p>';
                return;
            }
            ...
            specificRules.forEach(rule => {
                ...
                listContainer.innerHTML += `
                    <div class="geo-item flex items-center justify-between p-2 hover:bg-white/5 rounded-lg group transition-colors ${borderClass}" ${bgStyle} data-code="${rule.country_code.toLowerCase()}">
                        <div class="flex items-center gap-3">
                            <span class="text-xl">${getFlagEmoji(rule.country_code)}</span>
                            <span class="text-sm font-medium theme-text-main uppercase">${rule.country_code}</span>
                        </div>
        ...
        async function updateCountryStatus(countryCode, status) {
            const payload = { country_code: countryCode, status: status };
            const res = await apiRequest(`${BASE_URL}/geo-rules/country`, 'POST', payload);
            if(res) fetchGeoRules();
        }

        async function updateDefaultPolicy(policy) {
            const res = await apiRequest(`${BASE_URL}/geo-rules/default-policy`, 'PATCH', { policy: policy });
            if(res) {
                alert(`Default policy updated to: ${policy.toUpperCase()}`);
                fetchGeoRules();
            }
        }
        ...
        function updateDefaultUI(status) {
            ...
            if (status === 'allowed') {
                btnAllow.style.backgroundColor = "rgb(34, 197, 94)";
                btnAllow.style.color = "white";
            } else {
                btnDeny.style.backgroundColor = "rgb(239, 68, 68)";
        ...
        function togglePanicMode() {
            if(confirm("DANGER: This will immediately block all IP addresses outside your current default rules. Are you sure?")) {
                updateDefaultPolicy('deny_all').then(() => {
                    alert("🚨 Panic Mode Activated. Foreign traffic blocked.");
                });
            }
        }

        function filterGeoRules(e) {
            const query = e.target.value.toLowerCase();
            const items = document.querySelectorAll('.geo-item');
            items.forEach(item => {
                const code = item.getAttribute('data-code');
                item.style.display = code.includes(query) ? 'flex' : 'none';
            });
        }
    </script>
@endpush
